...
impl BinRead for TexFile {
    ...
    fn read_options<R: Read + Seek>(
        ...
        let mut texture_data_size: Vec<u32> =
            vec![0u32; min(file.header.mip_levels as u32, 13) as usize];
        ...
        for i in 0..file.header.mip_levels as usize {
            ...
            let mut size = (((file.header.height as u32) * (file.header.width as u32) * 4)
...
impl TexFile {
    ...
    pub fn from_dds(filename: String) -> Option<TexFile> {
        ...
        let mut txf = TexFile::new(
            ...
            dds.get_num_mipmap_levels() as u16,
        );
        let mut reader = Cursor::new(dds.get_data(0).expect("Error reading layer 0"));
        txf.header.mip_levels = dds.get_num_mipmap_levels() as u16;
        for i in 0..dds.get_num_mipmap_levels() {
            let mut tex =
                vec![0u8; ((dds.get_height() * dds.get_width() * 4) / (4u32.pow(i))) as usize];
            reader
    ...
    pub fn push_tex(&mut self, rgba: &[u8]) {
        let mut dst = vec![
            0u8;
            ((self.header.width as u32 * self.header.height as u32 * 4)
    ...
    pub fn write_dds(&mut self, filename: String) {
        let mut dds = Dds::new_d3d(ddsfile::NewD3dParams {
            ...
            mipmap_levels: Some(min(self.header.mip_levels as u32, 13)),
            caps2: None,
        })
